...
public class AutoShootWithIndex extends CommandBase {
   ...
   */
  public AutoShootWithIndex(Shooter Shooter) {
    shooter = Shooter;
  ...
  public void initialize() {
      timer.reset();
      timer.start();
      shooter.shoot(AutoConstants.Auto_Shoot_Power);
  }
 
  //this waits until the shooter is charged up, then launches the balls
  @Override
  public void execute() {
      if(timer.get() >= AutoConstants.Auto_Charge_Delay)
      {
  ...
  public boolean isFinished() {
    if(timer.get() >= AutoConstants.Auto_Shoot_EndTime)
        return true;
    return false;
  }
}
